...
//** Edit these methods to fit your specs and add them to "targets/type_checker.cpp"
...
void fir::type_checker::do_function_call_node(fir::function_call_node *const node, int lvl) {
  ...
  if (symbol->is_typed(cdk::TYPE_STRUCT)) {
    ...
    auto return_symbol = fir::make_symbol(false, symbol->qualifier(), symbol->type(), return_var_name, false, false);
...
//may have problems with _function on type checker.h
...
void fir::type_checker::do_identity_node(fir::identity_node *const node, int lvl) {
  // ASSERT_UNSPEC;
  // node->argument()->accept(this, lvl + 2);
  // if(node->argument()->type()->name() == cdk::basic_type::TYPE_INT) {
  //   node->type(new basic_type(4, cdk::basic_type::TYPE_INT));
  // }
  // else if(node->argument()->type()->name() == basic_type::TYPE_DOUBLE) {
  //   node->type(new basic_type(8, cdk::basic_type::TYPE_DOUBLE));
  // }
}
...
namespace fir {
  ...
  private:
    ...
    // My favourite function. Always calling it.
  ...
  protected:
    ...
    void process_boolean(bool value, const std::string &name, int lvl) {
      os() << std::string(lvl, ' ') << "<" << name << ">" << boolToStr(value) << "</" << name << ">" << std::endl;
    }
